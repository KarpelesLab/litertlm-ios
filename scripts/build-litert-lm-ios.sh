...
collect_headers() {
    ...
    # These are generated by the Rust CXX bridge toolchain and may be in
    # deeply nested platform-transitioned directories or _virtual_includes.
    log "  Collecting Rust CXX bridge generated headers..."
    local found_rs=0
    # Search ALL of bazel-out for .rs.h files (they're rare, so no perf issue)
    while IFS= read -r f; do
        # Extract the runtime/... relative path
        local rel
        rel=$(echo "$f" | sed -n 's|.*/bin/\(runtime/.*\.rs\.h\)|\1|p')
        if [ -z "${rel}" ]; then
            # Try extracting from _virtual_includes path
            rel=$(echo "$f" | sed -n 's|.*_virtual_includes/[^/]*/\(runtime/.*\.rs\.h\)|\1|p')
        fi
        if [ -n "${rel}" ] && [ ! -f "${dest_headers}/${rel}" ]; then
            log "    Found .rs.h: ${rel} (from $f)"
            mkdir -p "${dest_headers}/$(dirname "$rel")"
            cat "$f" > "${dest_headers}/${rel}" 2>/dev/null || cp -L "$f" "${dest_headers}/${rel}" 2>/dev/null || true
            found_rs=1
        fi
    done < <(find "${execroot}/bazel-out" -name '*.rs.h' \
        -not -path '*-exec-*' \
        2>/dev/null)
    if [ "${found_rs}" -eq 0 ]; then
        log "    No .rs.h in bazel-out, checking broader..."
        # Search the ENTIRE output_base (includes action cache, sandbox, etc.)
        while IFS= read -r f; do
            local rel
            rel=$(echo "$f" | grep -oE 'runtime/components/rust/[^/]*\.rs\.h$' || true)
            if [ -n "${rel}" ] && [ ! -f "${dest_headers}/${rel}" ]; then
                log "    Found .rs.h in output_base: $f"
                mkdir -p "${dest_headers}/$(dirname "$rel")"
                cat "$f" > "${dest_headers}/${rel}" 2>/dev/null || true
                found_rs=1
            fi
        done < <(find "${output_base}" -name '*.rs.h' 2>/dev/null)
    ...
    if [ "${found_rs}" -eq 0 ]; then
        log "    WARNING: .rs.h truly not found anywhere, generating stub"
        # The CXX bridge header is generated by the Rust toolchain and may
        # end up in a location we can't find. Generate a minimal stub that
        # declares the opaque type used by prompt_template.h.
        mkdir -p "${dest_headers}/runtime/components/rust"
        cat > "${dest_headers}/runtime/components/rust/minijinja_template.rs.h" << 'RSSTUB'
// Auto-generated stub for CXX Rust bridge header.
// The real file is generated by the CXX toolchain during Rust compilation
// but lives in a Bazel output path that can't be extracted from the build tree.
#pragma once
